...
S3_BUCKET = os.getenv("S3_BUCKET","tonexlabs")
...
def get_s3_client():
    client_kwargs = {'region_name': os.getenv("AWS_REGION", "ap-south-1")}
...
TARGET_VOICES = {
    "divyankar": "examples/reference/divyankarv.wav",
...
async def generate_speech(request: VoiceConversionRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        logger.info(f"Converting voice {request.source_audio_key} to {request.target_voice}")
        ...
        try:
            s3_client.download_fileobj(S3_BUCKET, Key=request.source_audio_key, Fileobj=source_temp)
            source_temp.close()
        except Exception as e:
            os.unlink(source_temp.name)
            raise HTTPException(status_code=404, detail="Source audio not found")

        vc_wave, sr = process_voice_conversion(model=models, source=source_temp.name, target_name=target_audio_path, output=None)
