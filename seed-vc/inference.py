...
@torch.no_grad()
def process_voice_conversion(model, source, target_name, output, f0_condition=False, auto_f0_adjust=False, pitch_shift=0, diffusion_steps=25, length_adjust=1.0, inference_cfg_rate=0.7):
    model, semantic_fn, f0_fn, vocoder_fn, campplus_model, mel_fn, mel_fn_args = model
    ...
    if output:
        ...
        torchaudio.save(os.path.join(output, f"vc_{source_name}_{target_name}_{length_adjust}_{diffusion_steps}_{inference_cfg_rate}.wav"), vc_wave.cpu(), sr)

    return vc_wave.cpu(), sr
