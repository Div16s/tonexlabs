...
import { generateSoundEffect, generationStatus } from "~/actions/generate-speech";
...
export function SoundEffectsGenerator({ credits }: { credits: number }) {
  ...
  const handleGenerateSoundEffect = async () => {
    ...
    try {
      ...
      if (shouldShowThrottleAlert) {
        toast("You're hitting the limit! Requests beyond 3/min will be queued.", {
          icon: "⚠️"
        })
    ...
    const pollInterval = setInterval(() => {
      void (async () => {
        try {
          ...
          if (stats.success && stats.audioUrl) {
          ...
          console.error("Error polling for sound effect generation status: ", error);
        }
      })();
    }, 500);
  ...
  }, [currAudioId, loading, playAudio, textContent]);
    ...
    <>
      ...
      <div className="relative flex h-full w-full flex-col items-center">
        <div className="absolute top-1/2 right-0 left-0 -translate-y-1/2 transform">
          <div
            className="h-[200px] w-full bg-gradient-to-r from-purple-400 via-blue-300 to-teal-300 opacity-20 blur-[70px]"
        ...
        <div className="relative z-10 flex h-full w-full flex-col items-center gap-10 md:pt-20">
          <div
            className={`h-fit w-full max-w-2xl rounded-xl border bg-white p-4 shadow-xl transition-colors duration-300 ${isFocused ? "border-black" : "border-gray-200"}`}
          >
            <div className="flex flex-col">
              <textarea
                ...
                onChange={(e) => {
                  ...
                  if (text.length <= MAX_CHARACTERS) setTextContent(text);
                }}
                onFocus={() => setIsFocused(true)}
                onBlur={() => setIsFocused(false)}
                maxLength={MAX_CHARACTERS}
                placeholder={activePlaceholder}
                className="h-16 resize-none rounded-md p-2 placeholder:font-light placeholder:text-gray-500 focus:border-none focus:ring-0 focus:outline-none"
              />
          ...
          <div className="h-fit w-full max-w-2xl rounded-xl border border-gray-200 bg-white p-4 shadow-lg">
            <p className="mb-2 text-center text-sm text-gray-500">Try a sound effect example</p>
